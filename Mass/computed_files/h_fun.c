...
CASADI_SYMBOL_EXPORT casadi_int h_fun_n_in(void) { return 1;}
...
void mex_h_fun(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  casadi_real* res[1] = {0};
  if (argc>1) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"h_fun\" failed. Too many input arguments (%d, max 1)", argc);
...
casadi_int main_h_fun(casadi_int argc, char* argv[]) {
  casadi_int j;
  casadi_real* a;
  const casadi_real* r;
  casadi_int flag;
...
CASADI_SYMBOL_EXPORT int main(int argc, char* argv[]) {
  if (argc<2) {
    /* name error */
  } else if (strcmp(argv[1], "h_fun")==0) {
    return main_h_fun(argc-2, argv+2);
  }
  fprintf(stderr, "First input should be a command string. Possible values: 'h_fun'\nNote: you may use function.generate_input to create a command string.\n");
  return 1;
}
#ifdef __cplusplus
} /* extern "C" */
